...
const ENVELOPE: EnvelopeCase[] = [
  ...
  { id: 'RC-E23', kind: 'envelope', title: 'decision_ref as a one-element array', base: 'decision', mutate: { decision_ref: [hex('c')] }, accepts: false,
    lines: 'L984-985, L1652-1660', note: 'decision_ref is a 64 lowercase hexadecimal digest, a string. It is conditional on the stage, so it is exercised on a record that carries it.' },
  { id: 'RC-E24', kind: 'envelope', title: 'prev as a one-element array', base: 'decision', mutate: { prev: [hex('d')] }, accepts: false,
    lines: 'L986-987, L1652-1660', note: 'prev is the receipt_id of another record, a string. Same reason for using a record that carries it.' },
  { id: 'RC-E25', kind: 'envelope', title: 'a signature value as a one-element array', base: 'intent',
    mutate: { signatures: [{ signer: AGENT, key_id: `${AGENT}#key-1`, alg: 'Ed25519', value: ['0'.repeat(128)] as unknown as Json }] }, accepts: false,
    lines: 'L975, L1652-1660', note: 'A signature value is 128 lowercase hexadecimal characters, a string. The one-element array of that string coerced through the pattern test.' },
...
const STAGE: StageCase[] = [
  ...
  { id: 'RC-S07', provenance: 'ruling-derived', kind: 'stage', title: 'An action intent under another result profile', base: 'intent', mutate: { 'result.profile': 'aps-action-intent-result-v2' },
  ...
  { id: 'RC-S24', provenance: 'ruling-derived', kind: 'stage', title: 'A decision result under another profile', base: 'decision', mutate: { 'result.profile': 'aps-core-decision-output-v2' }, context: { boundary_identity: BOUNDARY },
    expect: { status: 'invalid', stage: 'policy-decision', boundary_identity: 'verified', failure_codes: ['DECISION_RESULT_INVALID'] },
  ...
  { id: 'RC-S36', provenance: 'ruling-derived', kind: 'stage', title: 'An action result under another result profile', base: 'result', mutate: { 'result.profile': 'aps-action-result-v2' }, context: { boundary_identity: BOUNDARY },
